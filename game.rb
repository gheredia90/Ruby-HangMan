...
class Game
	...
	def add_character_to_partial_guess(character)
		...
		positions = (0 ... target_word_downcased.length).find_all { 
			|i| target_word_downcased[i,1] == character
		}
		@guesser.guessed_characters += positions.size
		positions.each do |i| 
	...
	def initialize_two_players
		...
		target_word = STDIN.noecho(&:gets).chomp
	...
	def check_play_status
		if @guesser.reached_attempts_limit?
			puts "Congrats #{@thinker.name}, you have won!"
			@playing = false
		end	

		if @guesser.guessed_characters == @target_word.size
			puts "Congrats #{@guesser.name}, you have guessed the word!"
			@playing = false
		end	
	end	
